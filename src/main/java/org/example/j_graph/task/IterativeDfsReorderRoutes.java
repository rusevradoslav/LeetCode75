 ...
 *       at index b.</li>
 *   <li>Run DFS from city 0 using an explicit stack, visiting every unvisited neighbor.</li>
 ...
 *     0 ←(orig)— 4 —(orig)→ 5
 *
 *   DFS from 0 (stack order may differ from recursive):
 *     visit 4  (via rev   0←4)  counter=0
 *     visit 5  (via orig  4→5)  counter=1
 *     visit 1  (via orig  0→1)  counter=2
 *     visit 3  (via orig  1→3)  counter=3
 *     visit 2  (via rev   3←2)  counter=3
 *   result: 3
 * </pre>
 *
 * <p>Time Complexity: O(n) — each city and edge visited once.
 *
 * <p>Space Complexity: O(n) — adjacency list, visited array, and stack each hold
 * at most n entries.
 */
public class IterativeDfsReorderRoutes implements ReorderRoutes {
